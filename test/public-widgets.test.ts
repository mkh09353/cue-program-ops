...
test("saved embed config CRUD persists and filters public sessions",async()=>{const local=createApp({repo:new MemoryRepository()}),h={"content-type":"application/json","x-demo-persona":"org-swyx"};const made=await local.request("/api/events/evt-ai-summit-2026/embed-configs",{method:"POST",headers:h,body:JSON.stringify({name:"Product only",widget:"sessions",filters:{track:"Product"},theme:{accent:"black"}})});assert.equal(made.status,201);const config=(await made.json()).data;const list=await(await local.request("/api/events/evt-ai-summit-2026/embed-configs",{headers:h})).json();assert.ok(list.data.some((x:any)=>x.id===config.id));const page=await(await local.request(`/e/ai-engineer-summit/public/sessions?config=${config.id}`)).text();assert.match(page,/Shipping AI Products/);assert.doesNotMatch(page,/Analytical Engines in Practice/);assert.equal((await local.request(`/api/events/evt-ai-summit-2026/embed-configs/${config.id}`,{method:"DELETE",headers:h})).status,204)});
