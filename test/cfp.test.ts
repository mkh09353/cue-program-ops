...
test("add fields plus publish payload renders public schema with conditional logic",async()=>{const app=createApp(),original=structuredClone(store.form),fields=[...store.form.fields,{key:"key_takeaway_fixture",label:"Key takeaway",type:"text" as const,required:true},{key:"audience_fixture",label:"Audience level",type:"select" as const,required:true,options:["Beginner","Intermediate","Advanced"]},{key:"prereq_fixture",label:"Workshop prerequisites",type:"textarea" as const,required:true,visibleWhen:{key:"format",equals:"Workshop (120 min)"}}];const saved=await app.request(`/api/events/${store.event.id}/forms/${store.form.id}`,{method:"PUT",headers:organizer,body:JSON.stringify({...store.form,fields,status:"open"})});assert.equal(saved.status,200);const publicForm=(await(await app.request(`/api/public/events/${store.event.slug}/cfp`)).json()).data.form;assert.equal(publicForm.fields.find((x:any)=>x.key==="key_takeaway_fixture").label,"Key takeaway");assert.deepEqual(publicForm.fields.find((x:any)=>x.key==="audience_fixture").options,["Beginner","Intermediate","Advanced"]);assert.deepEqual(publicForm.fields.find((x:any)=>x.key==="prereq_fixture").visibleWhen,{key:"format",equals:"Workshop (120 min)"});store.form=original});

test("organizer closeAt update closes public CFP immediately",async()=>{const app=createApp(),original=structuredClone(store.form);const changed=await app.request(`/api/events/${store.event.id}/forms/${store.form.id}`,{method:"PUT",headers:organizer,body:JSON.stringify({...store.form,status:"open",closeAt:"2000-01-01T00:00:00.000Z"})});assert.equal(changed.status,200);const body=await(await app.request(`/api/public/events/${store.event.slug}/cfp`)).json();assert.equal(body.data.window.open,false);assert.equal(body.data.window.reason,"Submissions closed");store.form=original});

test("public CFP exposes tracks only from current category field",async()=>{const app=createApp(),original=structuredClone(store.form);store.form.welcomeMd="Welcome\n\nTracks: Engineering · Product · Workshop · Agents";const body=await(await app.request(`/api/public/events/${store.event.slug}/cfp`)).json(),options=body.data.form.fields.find((x:any)=>x.key==="category").options;assert.doesNotMatch(body.data.form.welcomeMd,/Tracks:/i);assert.deepEqual(body.data.categories,options);assert.ok(body.data.form.routes.every((x:any)=>options.includes(x.category)));store.form=original});
...
test("co-presenters round-trip and accepted handoff includes every speaker",async()=>{const app=createApp();const body=valid(`lead-${Date.now()}@example.test`);(body.answers as any).additionalSpeakers=[{name:"Co Presenter",email:`co-${Date.now()}@example.test`}];const made=await post(app,`/api/public/events/${store.event.slug}/submissions`,body);const data=await made.json(),sub=store.submissions.find(x=>x.id===data.data.id)!;assert.equal(sub.additionalSpeakers?.[0].name,"Co Presenter");await post(app,`/api/events/${store.event.id}/submissions/${sub.id}/decision`,{nextStatus:"accepted"},organizer);const schedule=await(await app.request(`/api/events/${store.event.id}/schedule`,{headers:organizer})).json(),session=schedule.sessions.find((x:any)=>x.acceptedSubmissionId===sub.id);assert.deepEqual(session.speakerIds.sort(),[sub.speakerId,sub.additionalSpeakers![0].id].sort());assert.ok(store.personas.some(p=>p.id===sub.additionalSpeakers![0].id&&p.role==="speaker"))});
