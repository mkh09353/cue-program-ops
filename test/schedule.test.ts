...
test("capacity is warning and accepted unscheduled sessions are visible",()=>{const candidate={id:"preview",sessionId:"ses-product",roomId:"room-community",startsAt:"2026-10-12T19:00:00.000Z",endsAt:"2026-10-12T19:45:00.000Z"};assert.equal(validateSlot(demoSchedule,candidate).conflicts[0]?.code,"CAPACITY");assert.equal(scheduleWarnings(demoSchedule).map(x=>x.relatedIds[0]).join(","),"ses-reliable-agents,ses-sam,ses-workshop")});
test("canonical projection only exposes eligible public speakers",()=>{assert.deepEqual(publicSpeakers(demoSchedule).map(x=>x.id),["spk-ada","spk-sam"]);const out=canonicalFromSchedule("evt-ai-summit-2026",demoSchedule);assert.ok(out.sessions.every(x=>x.startsAt.startsWith("2026-10-12")))});
