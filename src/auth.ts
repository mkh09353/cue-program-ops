...
export interface AuthApiToken { id:string; tokenHash:string; userId:string; name:string; createdAt:string; lastUsedAt?:string; revokedAt?:string }
export interface AuthState { users:AuthUser[]; sessions:AuthSession[]; orgs:AuthOrganization[]; orgMemberships:AuthOrgMembership[]; eventRoles:AuthEventRole[]; invitations:AuthInvitation[]; magicLinkTokens:MagicLinkToken[]; apiTokens:AuthApiToken[] }
export interface AuthPrincipal { user:AuthUser; session:AuthSession; renewed?:boolean }

const DAY=86_400_000,SESSION_DAYS=7;
...
export function emptyAuthState():AuthState{return {users:[],sessions:[],orgs:[],orgMemberships:[],eventRoles:[],invitations:[],magicLinkTokens:[],apiTokens:[]}}
...
export function ensureSeededAuth(){
  ...
  if(!authStore.orgs.some(x=>x.id==="org-cue-demo"))authStore.orgs.push({id:"org-cue-demo",name:"Ruckus Demo Organization"});
...
export function createAuthRoutes(deps:{mailer:Mailer;persist:()=>Promise<void>;origin?:(request:Request)=>string}){
  ...
  app.post("/api/auth/magic-link",async c=>{const b=await c.req.json().catch(()=>null) as any,email=normalizedEmail(b?.email);if(!authStore.users.some(x=>x.email===email))return c.json({data:{accepted:true}});const token=randomToken(),expiresAt=new Date(Date.now()+15*60_000).toISOString();authStore.magicLinkTokens.push({tokenHash:await tokenHash(token),email,expiresAt});const origin=deps.origin?.(c.req.raw)||new URL(c.req.url).origin,url=`${origin}/login?token=${encodeURIComponent(token)}`;const subject="Your Ruckus sign-in link",text=`Sign in to Ruckus: ${url}\n\nThis link expires in 15 minutes.`;const result=await deps.mailer.send({to:email,subject,text,html:brandedHtmlFor(subject,text,{kind:"custom",ctaUrl:url,ctaLabel:"Open your sign-in link"})}).catch(()=>({status:"failed" as const}));await deps.persist();return c.json({data:{accepted:true,delivery:result.status,...(result.status==="mock_sent"?{demoOnlyLoginUrl:url,demoOnly:true}:{})}})});
  ...
  const organizer=async(c:any)=>{const principal=await current(c);if(!principal)return undefined;const allowed=authStore.orgMemberships.some(x=>x.userId===principal.user.id&&(x.role==="owner"||x.role==="admin"))||authStore.eventRoles.some(x=>x.userId===principal.user.id&&x.role==="organizer");return allowed?principal:undefined};
  app.post("/api/auth/tokens",async c=>{const principal=await organizer(c);if(!principal)return error(c,"authenticated organizer required",403);const b=await c.req.json().catch(()=>({})) as any,name=String(b?.name||"").trim();if(!name)return error(c,"token name required");const token=randomToken(),row:AuthApiToken={id:`token-${crypto.randomUUID()}`,tokenHash:await tokenHash(token),userId:principal.user.id,name,createdAt:new Date().toISOString()};authStore.apiTokens.push(row);await deps.persist();return c.json({data:{token,apiToken:{id:row.id,name:row.name,createdAt:row.createdAt}}},201)});
  app.get("/api/auth/tokens",async c=>{const principal=await organizer(c);if(!principal)return error(c,"authenticated organizer required",403);return c.json({data:authStore.apiTokens.filter(x=>x.userId===principal.user.id).map(({tokenHash,...row})=>row)})});
  app.delete("/api/auth/tokens/:id",async c=>{const principal=await organizer(c);if(!principal)return error(c,"authenticated organizer required",403);const row=authStore.apiTokens.find(x=>x.id===c.req.param("id")&&x.userId===principal.user.id);if(!row)return error(c,"API token not found",404);row.revokedAt=row.revokedAt||new Date().toISOString();await deps.persist();return c.json({data:{id:row.id,revokedAt:row.revokedAt}})});
  app.post("/api/auth/invitations",async c=>{const principal=await current(c);if(!principal)return error(c,"authentication required",401);const b=await c.req.json().catch(()=>null) as any,email=normalizedEmail(b?.email),orgId=b?.orgId?String(b.orgId):undefined,eventId=b?.eventId?String(b.eventId):undefined,role=String(b?.role||"member");const orgAdmin=orgId&&authStore.orgMemberships.some(x=>x.userId===principal.user.id&&x.orgId===orgId&&(x.role==="owner"||x.role==="admin")),eventAdmin=eventId&&(authStore.eventRoles.some(x=>x.userId===principal.user.id&&x.eventId===eventId&&x.role==="organizer")||authStore.orgMemberships.some(x=>x.userId===principal.user.id&&(x.role==="owner"||x.role==="admin")));if(!email||(!orgAdmin&&!eventAdmin))return error(c,"organization or event admin required",403);const token=randomToken(),row:AuthInvitation={tokenHash:await tokenHash(token),email,orgId,eventId,role,status:"pending",createdAt:new Date().toISOString(),expiresAt:new Date(Date.now()+7*DAY).toISOString()};authStore.invitations.push(row);const url=`${new URL(c.req.url).origin}/login?invitation=${encodeURIComponent(token)}`,subject="You are invited to Ruckus",text=`Accept your Ruckus invitation: ${url}`,result=await deps.mailer.send({to:email,subject,text,html:brandedHtmlFor(subject,text,{kind:"custom",ctaUrl:url,ctaLabel:"Open your invitation"})}).catch(()=>({status:"failed" as const}));await deps.persist();return c.json({data:{invitation:{email,orgId,eventId,role,status:row.status},delivery:result.status,...(result.status==="mock_sent"?{demoOnlyAcceptUrl:url,demoOnly:true}:{})}},201)});
