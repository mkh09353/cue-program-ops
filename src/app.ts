...
export interface AppDeps {
  ...
  demoMcpToken?: string;
  /** Scheduled automation uses the real mail provider only when explicitly enabled; the safe default logs reminders without provider I/O so cron runs can never exhaust a mail quota. */
  automationProviderDelivery?: boolean;
}
...
export function createApp(deps: AppDeps = {}) {
  ...
  app.post("/api/internal/automation/run",async(c)=>{
    ...
    // Provider-quota safety: scheduled runs use the mock mailer unless delivery is explicitly enabled,
