...
export function createApp(deps: AppDeps = {}) {
  ...
  app.get("/api/events/:eventId/bootstrap", (c) => {
    ...
    return c.json({
      data: {
        event: store.event,
        settings: { speakerConfirmation: store.event.speakerConfirmation !== false },
        actor: actor(c),
        personas,
        form: store.form,
        boards: store.boards,
...
export async function restoreSnapshot(deps: { repo: Repository; persistence: SnapshotPersistence }) {
  ...
  const hydrate = (into: typeof store, snapshot: CompetitionSnapshot) => {
    ...
    into.embedConfigs||=[];
    for(const embed of into.embedConfigs){if(embed.enabled===undefined)embed.enabled=true;if(!["iframe","script","link"].includes(embed.snippetFormat||""))embed.snippetFormat="iframe";if(embed.customCss===undefined)embed.customCss=""}
