...
{editing?<Card className="fixed inset-x-4 top-20 z-50 mx-auto max-h-[80vh] max-w-2xl overflow-auto p-6 shadow-xl" data-testid="session-editor">
  ...
  <div className="mb-4 rounded-[18px] border-2 border-ink bg-white p-3" data-testid="session-history-always-visible">
    ...
    {[...(editing.history||[])].sort((a:any,b:any)=>String(b.createdAt).localeCompare(String(a.createdAt))).slice(0,2).map((h:any)=><div key={`pinned-${h.id}`} className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded bg-soft p-2 text-xs"><span><b>{h.editorName}</b> · <span className="font-mono">{historyStamp(h.createdAt)}</span></span><Button size="sm" variant="outline" onClick={async()=>{const r:any=await api.restoreContentHistory(h.id);setEditing({...r.data});setSavedSession({id:r.data.canonicalId||r.data.id,title:r.data.title,contentStatus:r.data.contentStatus,at:historyStamp(r.data.restoredAt),stamp:Date.now(),restored:true});toast(`Restored content from ${historyStamp(h.createdAt)}`);load()}}>Restore this version</Button></div>)}
  ...
  <div className="flex flex-wrap items-center justify-between gap-2">
    ...
    <Button size="sm" variant="outline" aria-label={`Restore version from ${historyStamp(h.createdAt)}`} onClick={async()=>{const r:any=await api.restoreContentHistory(h.id);setEditing({...r.data});setSavedSession({id:r.data.canonicalId||r.data.id,title:r.data.title,contentStatus:r.data.contentStatus,at:historyStamp(r.data.restoredAt),stamp:Date.now(),restored:true});toast(`Restored the version saved at ${historyStamp(h.createdAt)}`);load()}}>Restore this version</Button>
  </div>
