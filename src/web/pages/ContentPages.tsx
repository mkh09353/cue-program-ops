...
{archiveOpen?<Card className="fixed inset-x-4 top-16 z-50 mx-auto max-h-[80vh] max-w-2xl overflow-auto p-6 shadow-xl" data-testid="archive-dialog">
  ...
  <div className="mt-3 grid gap-3 md:grid-cols-2">
    ...
    <div>
      ...
      <div className="mt-2 max-h-52 space-y-1 overflow-auto">
        {data.files.map((f:any)=>{const checked=archiveFiles.includes(f.id);return <label key={f.id} className={`flex cursor-pointer items-center gap-2 rounded-[18px] border px-3 py-2 text-sm ${checked?"border-ink bg-soft":"border-line bg-white"}`}><input type="checkbox" aria-label={`Include file ${f.currentVersion?.name||f.id}`} checked={checked} onChange={e=>setArchiveFiles(prev=>e.target.checked?[...new Set([...prev,f.id])]:prev.filter(x=>x!==f.id))}/><span className="min-w-0"><b className="block truncate">{f.currentVersion?.name||f.id}</b><span className="block truncate text-xs text-mid">{f.speaker?.name||"No speaker"} · {f.session?.title||"General"} · v{f.currentVersion?.version??f.versions.length}</span></span></label>})}
        {!data.files.length?<p className="text-xs text-mid">No uploaded files yet.</p>:null}
      </div>
    </div>
  </div>
  {exportState?<Notice tone={exportState.status==="ready"?"ok":exportState.status==="failed"?"danger":"info"} onClose={()=>setExportState(null)}>{exportState.status==="working"?"Building ZIP…":exportState.status==="failed"?exportState.error:<span><b>Download started</b> · {exportState.filename} · {exportState.fileCount} file{exportState.fileCount===1?"":"s"} · grouped by {exportState.grouping} · {exportState.sizeKb} KB · {exportState.at}</span>}</Notice>:null}
  <div className="mt-4 flex flex-wrap gap-2">
    <Button disabled={exportBusy||(!archiveSessions.length&&!archiveFiles.length)} data-testid="archive-download" onClick={async()=>{
      ...
      try{
        ...
        const url=URL.createObjectURL(r.blob);const a=document.createElement("a");a.href=url;a.download=r.filename;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),4000);
        setExportState({status:"ready",fileCount:r.fileCount,grouping:r.grouping,filename:r.filename,sizeKb:Math.max(1,Math.round(r.blob.size/1024)),at:new Date().toLocaleTimeString()});
    ...
    }}>{exportBusy?"Building ZIP…":"Download archive"}</Button>
...
{editing?<Card className="fixed inset-x-4 top-20 z-50 mx-auto max-h-[80vh] max-w-2xl overflow-auto p-6 shadow-xl" data-testid="session-editor">
  ...
  <div className="flex flex-wrap items-center justify-between gap-2">
    ...
    <Button size="sm" variant="outline" aria-label={`Restore version from ${historyStamp(h.createdAt)}`} onClick={async()=>{await api.restoreContentHistory(h.id);toast(`Restored the version saved at ${historyStamp(h.createdAt)}`);setEditing(null);load()}}>Restore this version</Button>
  </div>
...
{speaker?<Card className="fixed inset-x-4 top-20 z-50 mx-auto max-w-xl overflow-auto max-h-[85vh] p-6 shadow-xl" data-testid="speaker-editor"><PageHeader title={`Edit ${speaker.name}`} description={`Speaker ${speaker.speakerId}`}/>
  <div className="mb-3 flex items-center gap-3 rounded-[18px] border border-line bg-soft p-3">
    {speaker.headshotUrl?<img src={speaker.headshotUrl} alt={`${speaker.name} headshot`} data-testid="speaker-headshot-preview" className="h-20 w-20 rounded-[18px] border border-line bg-white object-contain" onError={()=>setHeadshotState("error")}/>:<div className="grid h-20 w-20 place-items-center rounded-[18px] border border-line bg-white text-sm font-semibold text-mid">{speaker.name?.split(" ").map((x:string)=>x[0]).join("").slice(0,2)}</div>}
  ...
  {speaker.headshotUrl?<div className="rounded-[18px] border border-line bg-soft p-3">
    ...
    <img src={speaker.headshotUrl} alt={`${speaker.name} headshot preview`} data-testid="headshot-large-preview" className="h-40 w-40 rounded-[18px] border border-line bg-white object-contain"
